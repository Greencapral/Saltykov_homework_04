### Как заставить всё это заработать

#### 1. Формируем правильную среду исполнения
- при необходимости установить uv (команда **pip install uv**)
- все необходимые зависимости описаны в **pyproject.toml**, накатываем командой **uv sync**

#### 2. Поднимаем СУБД
- при разработке использовался докер-образ **postgres:18.1-alpine**
- параметры подключения указаны в **homework_04/models/config.py**
- если лень лезть, то вот они:
...
#### 3. Подготовка и запуск программы
- запуск осуществляется в модуле **homework_04/main.py**
- при необходимости можно изменить константы:
